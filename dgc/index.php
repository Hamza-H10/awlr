<?php 
  ...
  if($page_action == "login")  {
      ...
      $token = md5($password); //hashPassword($password);
      $stmt = $database->execute("select user_id as id, user_name, role from tbl_users where user_email='$email' and user_password='$token'");
      ...
      if($stmt->rowCount() >= 1)  {
          if($Results = $stmt->fetch(PDO::FETCH_ASSOC)) {
              ...
              $_SESSION['usertype'] = $session_user_type = $Results['role'];
              $logged_in = TRUE;
          }
      }
      else
