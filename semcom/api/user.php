...
EOD;
        ...
        elseif($redirect == "vw_list") 
            $query = "SELECT id as row_id, slave_no, channel_no, `data`, date(record_time) as record_time FROM vwire_dd $search_text ORDER BY record_time DESC ";
        else 
...
//file_put_contents('logs.txt', "NUM=".$num.PHP_EOL , FILE_APPEND | LOCK_EX);
        ...
        if($num > 0) {
            ...
            elseif($redirect == "vw_list")
                $results_arr["text_align"]=array('', 'center', 'center', 'right');
            else
