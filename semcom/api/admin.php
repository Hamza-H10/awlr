<?php
    ...
    switch($redirect) {
        ...
        case "user_list":
            ...
            $stmt = $database->execute("SELECT id as row_id, user_name, user_email, user_type FROM users $search_text LIMIT $page_start, $page_limit");
            $num = $stmt->rowCount();
        ...
        case "inclsetup_edit":
            ...
            $stmt = $database->execute("UPDATE inplace_depth SET depth=$value WHERE id=".$row_id);
            $num = $stmt->rowCount();
            ...
            break;
        case "inclsetup_list":
            $slave_no = getValue("slave_no");
            $option = getValue("option", false, '');

            $stmt = $database->execute("SELECT id as row_id, channel_no, depth FROM inplace_depth WHERE slave_no=".$slave_no);
            $num = $stmt->rowCount();
