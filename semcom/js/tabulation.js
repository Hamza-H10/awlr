...
class Tabulation  {
    ...
    mDelFunc() {
        ...
        this.loadPage(1,true,this.del_url+ids+"&pgno="+this.cur_page_num);
    }

    mSingleDelFunc(rowid) {
        this.loadPage(1,true,this.del_url+rowid+"&pgno="+this.cur_page_num);
    }
        ...
        if( $('.ui.form').form('is valid')) {
            ...
            $( "#"+self.prefix+"_userform" ).serialize()).done( 
                ...
                if(myObj.status === "success") {
                    ...
                    $('#'+self.prefix+'_userform').form('reset');
                    self.loadPage(1, (self.edit_id == 0), self.load_api_url+self.cur_page_num);
    ...
    loadPage(pgno, reset=false, access_url='') {
        ...
        $.get(access_url, function(data, status) {
            ...
            if(self.loader_function)
                ...
                table_output = "<table class='ui celled compact striped brown table'><tr><td class='center aligned'>No records found</td></tr></table>";
